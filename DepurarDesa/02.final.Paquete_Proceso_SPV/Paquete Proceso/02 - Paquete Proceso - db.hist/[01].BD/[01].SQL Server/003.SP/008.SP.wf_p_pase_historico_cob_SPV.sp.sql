...
BEGIN  
	...
	SET @CboCommit  = isnull((select prt_valor FROM wf_parametros WHERE prt_nombre_corto = 'CboCommit' and prt_baja_fecha is null),0)
	SET @CboDias  = isnull((select prt_valor FROM wf_parametros WHERE prt_nombre_corto = 'CboDias' and prt_baja_fecha is null),999)
	...
	WHERE
		--Anteriores a N Dias
		DATEDIFF(DAY, a.cob_alta_fecha, @v_fec_proceso) > @CboDias
   ...
   INSERT wf_print_out SELECT @prc_nombre_corto,GETDATE(),@v_fec_proceso,'A','Cantidad de registros de bitacora de objetos por iteracion: '+ convert(varchar,@CboCommit), @v_usu_id, Null,@cat_id 
   INSERT wf_print_out SELECT @prc_nombre_corto,GETDATE(),@v_fec_proceso,'A','', @v_usu_id, Null,@cat_id  	
   ...
   BEGIN TRY 
		...
		BEGIN  
			...
			SELECT top (@CboCommit) 'wf_cmb_objetos', cob_id, cob_alta_fecha
			FROM wf_cmb_objetos as a
			WHERE
				--Anteriores a N Dias
				DATEDIFF(DAY, a.cob_alta_fecha, @v_fec_proceso) > @CboDias
			...
			BEGIN TRAN   
				...
				BEGIN  
					...
					DELETE wf_cmb_objetos FROM wf_cmb_objetos INNER JOIN tmp_borrar ON cob_id = id and cob_alta_fecha = cambio_fecha  and tabla = 'wf_cmb_objetos'
