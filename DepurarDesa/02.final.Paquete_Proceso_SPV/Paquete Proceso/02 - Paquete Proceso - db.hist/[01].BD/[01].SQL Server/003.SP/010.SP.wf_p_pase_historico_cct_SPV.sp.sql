...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[wf_p_pase_historico_cct_SPV]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[wf_p_pase_historico_cct_SPV]
go
CREATE PROCEDURE [dbo].[wf_p_pase_historico_cct_SPV] (  
...
BEGIN  
	...
	SET @CctCommit  = isnull((select prt_valor FROM wf_parametros WHERE prt_nombre_corto = 'CctCommit' and prt_baja_fecha is null),10000)
	SET @CctDias  = isnull((select prt_valor FROM wf_parametros WHERE prt_nombre_corto = 'CctDias' and prt_baja_fecha is null),36)
	...
	WHERE
	--Anteriores a N Dias
	DATEDIFF(DAY,a.cct_alta_fecha,@v_fec_proceso) > @CctDias
   ...
   BEGIN TRY 
		...
		BEGIN  
			...
			SELECT top (@CctCommit) 'cmb_cta', cct_id, cct_alta_fecha
			FROM cmb_cta a
			WHERE
			--Anteriores a N Dias
			DATEDIFF(DAY,a.cct_alta_fecha,@v_fec_proceso) > @CctDias
			...
			BEGIN TRAN   
				...
				BEGIN  
					...
					EXEC dbo.wf_p_chequear_dependencias_SPV @v_fec_proceso, 'cmb_cta', @v_accion, @prc_nombre_corto  

					DELETE cmb_cta FROM cmb_cta INNER JOIN tmp_borrar ON cct_id = id and cct_alta_fecha = cambio_fecha and tabla = 'cmb_cta'
		...
		UPDATE tablas_depurar_SPV SET atd_ultima_ejecucion = GETDATE() WHERE atd_tabla = 'cmb_cta'		
