USE Emerix
GO
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[wf_p_pase_historico_cob_BGP]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[wf_p_pase_historico_cob_BGP]
go
CREATE PROCEDURE [dbo].[wf_p_pase_historico_cob_BGP] (  
...
BEGIN  
	...
	SET @ObjCommit  = isnull((select prt_valor FROM wf_parametros WHERE prt_nombre_corto = 'ObjCommit' and prt_baja_fecha is null),0)
	SET @ObjMeses  = isnull((select prt_valor FROM wf_parametros WHERE prt_nombre_corto = 'ObjMeses' and prt_baja_fecha is null),999)
	...
	WHERE
		...
		DATEDIFF(mm,a.cob_alta_fecha,@v_fec_proceso) > @ObjMeses
   ...
   INSERT wf_print_out SELECT @prc_nombre_corto,GETDATE(),@v_fec_proceso,'A','Cantidad de registros de bitacora de objetos por iteracion: '+ convert(varchar,@ObjCommit), @v_usu_id, Null,@cat_id 
   INSERT wf_print_out SELECT @prc_nombre_corto,GETDATE(),@v_fec_proceso,'A','', @v_usu_id, Null,@cat_id  	
   ...
   BEGIN TRY 
		...
		BEGIN  
			...
			SELECT top (@ObjCommit) 'wf_cmb_objetos', cob_id
			FROM wf_cmb_objetos as a
			WHERE
				--Anteriores a N meses
			DATEDIFF(mm,a.cob_alta_fecha,@v_fec_proceso) > @ObjMeses
			...
			BEGIN TRAN   
				...
				BEGIN  
					...
					EXEC dbo.wf_p_chequear_dependencias_BGP 'wf_cmb_objetos', @v_accion, @prc_nombre_corto  
